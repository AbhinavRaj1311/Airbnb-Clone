...
/**
 * LightboxDynamic — accepts images prop directly (property-specific)
 * No static data imports — guaranteed correct images per property
 */
function LightboxDynamic({ isOpen, images = [], currentIndex, onClose, onNext, onPrev }) {
  ...
  // Lock body scroll, focus close btn
  useEffect(() => {
    if (isOpen) {
  ...
  useEffect(() => {
    ...
    const preloadUrls = []
    if (currentIndex > 0) preloadUrls.push(images[currentIndex - 1].src)
    if (currentIndex < images.length - 1) preloadUrls.push(images[currentIndex + 1].src)
    preloadUrls.forEach(url => { const img = new Image(); img.src = url })
  }, [isOpen, currentIndex, images, currentImage])
    ...
    >
      ...
      <div className={styles.header} onClick={e => e.stopPropagation()}>
        <div className={styles.counter} aria-live="polite" aria-atomic="true">
          <span className={styles.counterText}>{currentIndex + 1} / {total}</span>
        </div>
        <button
          ref={closeRef}
